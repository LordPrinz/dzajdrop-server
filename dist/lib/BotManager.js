...
var BotManager = /** @class */ (function () {
    function BotManager() {
        ...
        var _loop_1 = function (botToken) {
            var client = app_1.createNewBotInstance(botToken);
            // Save bot instances to bots array.
            client.login(botToken).then(function () {
                _this.bots.push(client);
                // Set the destination channel.
                if (!isDestinationChannelSet) {
                    client.channels.fetch(config_1.config.channelId).then(function (channel) {
    ...
    BotManager.prototype.getAvailableBot = function (requiredSlots) {
        ...
        var availableBotIndex = -1;
        for (var i = 0; i < this.bots.length; i++) {
            if (this.occupiedSlots[i] < config_1.config.botSlots &&
                this.occupiedSlots[i] + requiredSlots <= config_1.config.botSlots) {
                availableBotIndex = i;
                break;
            }
        }
        ...
        return {
            ...
            slots: config_1.config.botSlots - this.occupiedSlots[availableBotIndex],
        };
    };
